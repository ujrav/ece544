...
def generator(data, is_train, side_length, with_bn = True):
    """
    ...
    conv1 = tf.nn.relu(conv2d("g_conv1", rshp, [5, 5, 512, 1024], STRIDE_2,
                              top_shape=[batch_size, dim, dim, 512], is_train=is_train, with_bn=with_bn))

    # conv2 block, 256 outputs
    dim *= 2
    conv2 = tf.nn.relu(conv2d("g_conv2", conv1, [5, 5, 256, 512], STRIDE_2,
                              top_shape=[batch_size, dim, dim, 256], is_train=is_train, with_bn=with_bn))

    # conv3 block, 128 outputs
    dim *= 2
    conv3 = tf.nn.relu(conv2d("g_conv3", conv2, [5, 5, 128, 256], STRIDE_2,
                              top_shape=[batch_size, dim, dim, 128], is_train=is_train, with_bn=with_bn))

    # conv4 block, 64 outputs
    dim *= 2
    conv4 = tf.nn.relu(conv2d("g_conv4", conv3, [5, 5, 64, 128], STRIDE_2,
                              top_shape=[batch_size, dim, dim, 64], is_train=is_train, with_bn=with_bn))

    # conv5 block, 3 outputs
    conv5 = tf.nn.tanh(conv2d("g_conv5", conv4, [3, 3, 3, 64], STRIDE_1,
                              top_shape=[batch_size, dim, dim, 3], is_train=is_train, with_bn=with_bn))
...
def discriminator(data, is_train,reuse=False, with_bn=True):
    """
    ...
    # conv2 block, 256 outputs
    conv2 = lrelu(conv2d("d_conv2", conv1, [5, 5, 128, 256], STRIDE_2, is_train=is_train, with_bn=with_bn))

    # conv3 block, 512 outputs
    conv3 = lrelu(conv2d("d_conv3", conv2, [5, 5, 256, 512], STRIDE_2, is_train=is_train, with_bn=with_bn))

    # conv4 block, 1024 outputs
    conv4 = lrelu(conv2d("d_conv4", conv3, [5, 5, 512, 1024], STRIDE_2, is_train=is_train, with_bn=with_bn))
